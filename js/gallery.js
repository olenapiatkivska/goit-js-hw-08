...
function imageCollection(imgObj) {
...
function imagesCollection(arr) {
  return arr.map(imageCollection).join('');
}

const markup = imagesCollection(images);
console.log(markup);

listElem.insertAdjacentHTML('afterbegin', markup);

listElem.addEventListener('click', selectImage);
